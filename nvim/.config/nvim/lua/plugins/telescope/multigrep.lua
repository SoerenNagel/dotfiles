...
local live_multigrep = function(opts)
  ...
  local finder = finders.new_async_job {
    command_generator = function(prompt)
      ...
      return vim.iter(kwargs):flatten():totable()
      -- return vim.tbl_flatten(kwargs)
    end,
  ...
  pickers
    .new(opts, {
      debounce = 100,
      prompt_title = 'Multi Grep', --optional
      finder = finder,
      previewer = conf.grep_previewer(opts),
      sorter = require('telescope.sorters').empty(), --optional but reccomened
    })
    :find()
end
